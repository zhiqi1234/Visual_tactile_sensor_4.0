# -*- coding: utf-8 -*-
"""
V6_force_eval_ablation_lightnet.py — LightNet 消融实验对比评估
同时加载两个 LightNet 模型（Vision-only 和 Vision+Piezo），在同一图表中对比
"""
import os
import sys
import json
...
def load_model(model_dir, data_dir):
    ...
    model = LightNet(output_dim=config['output_dim'],
                     use_piezo=config.get('use_piezo', False)).to(device)
...
def plot_ablation_comparison(y_true, y_pred_base, y_pred_multi, columns, save_dir):
    ...
    fig.suptitle('消融实验对比 (LightNet): Vision-only vs Vision+Piezo', fontsize=14)
    fig.tight_layout()
...
def plot_ablation_error_comparison(y_true, y_pred_base, y_pred_multi, columns, save_dir):
    ...
    for idx, metric in enumerate(metric_names):
        ...
        ax.set_title(f'{metric} 对比 (LightNet)')
        ax.set_xticks(x)
        ax.set_xticklabels(columns)
        ax.legend()
        ax.grid(True, alpha=0.3, axis='y')

    fig.suptitle('消融实验：指标对比 (LightNet)', fontsize=14)
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="LightNet 消融实验对比评估")
    parser.add_argument('--data_dir', type=str, default=None)
    parser.add_argument('--baseline_dir', type=str, default=None,
                        help="LightNet Vision-only 模型目录")
    parser.add_argument('--multimodal_dir', type=str, default=None,
                        help="LightNet Vision+Piezo 模型目录")
    ...
    default_baseline = os.path.join(args.data_dir, "model_output_lightnet")
    default_multimodal = os.path.join(args.data_dir, "model_output_lightnet_piezo")
    ...
    if args.baseline_dir is None:
        if os.path.isdir(default_baseline):
            ...
            args.baseline_dir = filedialog.askdirectory(
                title="选择 LightNet Vision-only 模型目录", initialdir=args.data_dir)
            root.destroy()
    ...
    if args.multimodal_dir is None:
        if os.path.isdir(default_multimodal):
            ...
            args.multimodal_dir = filedialog.askdirectory(
                title="选择 LightNet Vision+Piezo 模型目录", initialdir=args.data_dir)
            root.destroy()

    if not args.baseline_dir or not args.multimodal_dir:
        print("未找到模型目录，请先运行 V6_force_train_lightnet.py 训练两个模型")
        return

    print("=" * 60)
    print("LightNet 消融实验对比评估")
    print("=" * 60)

    print("\n加载 LightNet Vision-only 模型...")
    model_base, scaler_base, dataset_base, test_idx_base, config_base, device = \
        load_model(args.baseline_dir, args.data_dir)

    print("\n加载 LightNet Vision+Piezo 模型...")
    ...
    eval_dir = os.path.join(args.data_dir, "ablation_comparison_lightnet")
